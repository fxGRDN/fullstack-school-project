...
const update_values = (comp: Element, data:CarData<string>) => {
...
const updateCarOwner = (comp:Element ,cData:CarData<string>) => {
    ...
    .then((data: UserData<string>[]) => {
        ...
        submit.addEventListener('change', (e) => {
            ...
            newOwner = data.findIndex( (element) => {
                if(element.id == caller.value) return true;
                return false;
            })
        });
